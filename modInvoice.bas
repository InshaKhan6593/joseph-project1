...
ErrHandler:
    TogglePerformance False
    ErrorHandler "GenerateInvoice", Err.Number, Err.Description
End Sub
...
Public Sub FinalizeInvoice()
    ...
    AuditLog "INVOICE_CREATED", invNo & " for " & modUtilities.FormatCurrency(total)
    TogglePerformance False
...
ErrHandler:
    TogglePerformance False
    ErrorHandler "FinalizeInvoice", Err.Number, Err.Description
End Sub
...
ErrHandler:
    modUtilities.ProtectSheet wsInv.Name
    ErrorHandler "ClearInvoiceTemplate", Err.Number, Err.Description
End Sub
...
Public Sub EditInvoice(invoiceNo As String)
    ...
    Exit Sub
ErrHandler:
    ErrorHandler "EditInvoice", Err.Number, Err.Description
End Sub
...
Public Sub CancelInvoice(invoiceNo As String)
    ...
    For i = 2 To wsTrans.Cells(wsTrans.Rows.Count, 1).End(xlUp).Row
        If CStr(wsTrans.Cells(i, 1).Value) = invoiceNo Then
            ...
            AuditLog "INVOICE_CANCELLED", invoiceNo
    ...
    Exit Sub
ErrHandler:
    ErrorHandler "CancelInvoice", Err.Number, Err.Description
End Sub
